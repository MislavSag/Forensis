...
# --- NOVE FUNKCIJE ZA ZKRH PRETRAGU PREKO MYSQL BAZE (umjesto MongoDB) ---

# Verzija zkrh funkcije koja koristi data.table::tstrsplit za brže parsiranje ID-a
zkrh_sql <- function(search_term, part, history = "false", limit = 100, skip = 0,
                     slop = 5) {
  ...
  if (is.null(res$hits) || length(res$hits) == 0) {
    ...
    # Koristimo `tstrsplit` iz data.table za direktno i efikasno
    # izdvajanje potrebnih stupaca iz 'id' stupca.
    # ISPRAVAK: Neki ID-jevi (npr. kod pretrage "Republika Hrvatska") imaju više od 3 crtice.
    # `tstrsplit` s opcijom `keep` je robustan način za izdvajanje samo potrebnih dijelova (2., 3. i 4.),
    # bez obzira na ukupan broj dijelova. Time izbjegavamo grešku "Supplied X columns to be assigned Y items".
    dt[, c("institutionId", "mainBookId", "lrUnitNumber") := tstrsplit(id, "-", fixed = TRUE, keep = 2:4)]

    return(dt)
  }
}

# Funkcija za dohvaćanje linkova iz nove MySQL baze "zkpdf"
dohvati_linkove_iz_zkpdf <- function(api_data) {
  # Provjera ima li podataka za dohvat
  if (nrow(api_data) == 0) {
    return(data.table())
  }

  # Povezivanje na bazu koristeći globalne varijable
  db <- dbConnect(MySQL(),
                  dbname = 'odvjet12_zk',
                  host = mysql_host,
                  port = mysql_port,
  ...
  # Osiguravanje UTF-8 konekcije
  dbSendQuery(db, 'SET NAMES utf8')

  # Izdvajanje jedinstvenih parova za upit radi efikasnosti
  ids_to_fetch <- unique(api_data[, .(mainBookId, lrUnitNumber)])

  # Kreiranje stringa za IN klauzulu, npr. "('id1', 'num1'), ('id2', 'num2')"
  id_parovi <- paste0("(", apply(ids_to_fetch, 1, function(row) {
    paste0("'", row[1], "', '", row[2], "'")
  }), ")", collapse = ",")

  # Sastavljanje upita
  query <- paste0(
    "SELECT mainBookId, lrUnitNumber, fileUrl FROM zkpdf WHERE (mainBookId, lrUnitNumber) IN (",
    id_parovi,
    ")"
  )

  # Dohvaćanje podataka i zatvaranje konekcije
  rezultati_sql <- dbGetQuery(db, query)
  dbDisconnect(db)

  return(as.data.table(rezultati_sql))
}

# Funkcija za spajanje podataka iz zkrh_sql i zkpdf
spoji_podatke_sql <- function(api_data, sql_data) {
  # Osiguravanje da su ključni stupci istog tipa (character)
  api_data[, `:=`(mainBookId = as.character(mainBookId), lrUnitNumber = as.character(lrUnitNumber))]
  sql_data[, `:=`(mainBookId = as.character(mainBookId), lrUnitNumber = as.character(lrUnitNumber))]

  # Spajanje tablica po mainBookId i lrUnitNumber
  spojena_tablica <- merge(api_data, sql_data, by = c("mainBookId", "lrUnitNumber"), all.x = TRUE)

  return(spojena_tablica)
}

# ovo je funkcija za dohvaćanje lrUnitId. To nam treba za web scrapanje pdf linkova
dohvati_lrUnitId_iz_mongo <- function(api_data) {
  if (nrow(api_data) == 0) {
    return(data.table())
  }
  ...
  conn <- NULL
  tryCatch({
    conn <- mongo(collection = collection_name_json, db = db_name, url = db_url)

    # Izdvajanje jedinstvenih parova za upit
    ids_to_fetch <- unique(api_data[, .(mainBookId, lrUnitNumber)])

    # Kreiranje liste uvjeta za $or operator
    conditions <- lapply(1:nrow(ids_to_fetch), function(i) {
      list(mainBookId = ids_to_fetch$mainBookId[i], lrUnitNumber = ids_to_fetch$lrUnitNumber[i])
    })

    # Sastavljanje upita
    query <- list('$or' = conditions)

    # Definiranje polja koja želimo dohvatiti (projekcija)
    fields <- '{"mainBookId": 1, "lrUnitNumber": 1, "lrUnitId": 1, "_id": 0}'

    # Izvršavanje upita
    documents <- conn$find(query = toJSON(query, auto_unbox = TRUE), fields = fields)

    return(as.data.table(documents))

  }, error = function(e) {
    message("Greška pri dohvaćanju lrUnitId iz MongoDB: ", e$message)
    return(data.table())
  }, finally = {
    if (!is.null(conn)) conn$disconnect()
  })
}
...
dohvati_token <- function() {
  response <- POST("https://sudreg-data.gov.hr/api/oauth/token",
                   ...
                   #authenticate("TApnnmx8utL-Y_CmMt-c8w..", "TApnnmx8utL-Y_CmMt-c8w.."),
...
# 08.04.2025. dodao sam nove funkije za nova plovila
DT_template_plovila1 <- function(df, caption = NULL, filename_prefix = "Plovila_") {
  DT::datatable(
    df,
    rownames = FALSE,
    caption = caption,
    class = "compact stripe",  # Manji padding, kompaktniji izgled
    ...
    options = list(
      ...
      buttons = list(
        ...
        list(
          extend = "csv",
          filename = paste0(filename_prefix, format(Sys.Date(), "%d-%m-%Y")),
          bom = TRUE,
          charset = "utf-8",
          exportOptions = list(
            modifier = list(page = "all"),
            columns = ":all"  # Eksportira sve stupce, uključujući skrivene
          )
        ),
        list(
          extend = "excel",
      ...
      columnDefs = list(
        list(
          targets = which(names(df) == "PodrucjePlovidbe") - 1,  # DataTables je zero-indexiran
          visible = FALSE
        )
      )
    )
...
DT_template_plovila2 <- function(df, caption = NULL, filename_prefix = "Export_") {
  if (nrow(df) > 0) {
    DT::datatable(
      df,
      rownames = FALSE,
